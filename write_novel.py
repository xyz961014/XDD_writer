...
def plan_subsubchapters(chapter_outlines, subchapter_outlines, subchapter_outline, recap, num_subsubchapters=3):
    ...
    请模仿以上给出的例子, 扩展以下子章节概要生成子章节段落概要, 每个段落概要至少应该包含三个具体的故事情节, 只用故事情节来表达子章节概要的内容, 不许写主人公学会、体会、明白、感悟、意识到等内容。请严格使用以上给定的输出格式进行输出: 
        子章节概要:
            {subchapter_outline}
    """
...
def write_subsubchapter(chapter_outlines, character_intro, recap, last_paragraph, subsubchapter_outline):
    ...
    f"""
    你是一位儿童文学作家, 正在创作一本主人公为小学男生许多多的儿童小说。
    请根据小说的段落概要和提供的其他信息使用生动的文字完成段落的创作, 创作时不用拘泥于段落概要的内容, 尽可能创造具体、生动、有趣的情节。
    ...
    段落内容的语言风格必须完全模仿之前给出的例子。请通过对话等方式发展小说情节, 思考什么样的情节是引人入胜的。请只用故事情节来表达段落概要的内容, 不要描述人物的感想和收获, 不描述事件造成的影响, 不对情节进行总结和升华。注意你正在创作一篇长篇小说, 故事才刚刚开始, 不要让情节发展得过快。你写的段落内容是紧接着上一段落展开的, 请确保语言和情节内容的流畅性。
...
def write_paragraph(idea, character_intro, recap, last_paragraph, paragraph_outline, instruction=""):
    ...
    续写段落内容的语言风格请完全模仿之前给出的例子。请通过对话等方式发展小说情节, 思考什么样的情节是引人入胜的。
    请只用故事情节来表达段落概要的内容, 不要描述人物的感想和收获, 不描述事件造成的影响, 不对情节进行总结和升华。
    注意你正在创作一篇长篇小说, 故事才刚刚开始, 不要让情节发展得过快。
    """
        if instruction.strip() != "":
